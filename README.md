...
模块一采用等价类、边界值、场景法及状态转换分析设计测试，使用 JUnit 5 实现自动化。模块二目前仅预留目录，尚未开展。
...
`sut` 是从原毕设系统提取的代码快照，测试工程直接编译该目录；无需访问原毕设目录。它不是完整的前后端部署工程，登录页面和真实跨链业务演示需在原毕设系统进行。来源和提取边界见 [SUT 说明](module1-basic-testing/sut/README.md)。
...
请保留完整仓库结构，因为 `tests/pom.xml` 引用了相邻的 `sut/src/main/java`。
...
```bash
cd module1-basic-testing/tests
bash ./run-tests.sh
```

### 直接使用 Maven

在已配置 JDK 17 和 Maven 的终端中，从仓库根目录执行：
...
也可以从仓库根目录运行 `mvn -f module1-basic-testing/tests/pom.xml clean test`。Windows 原生 PowerShell 可使用 Maven 命令，无需 Bash；本仓库现有执行证据来自 WSL 环境。
...
执行结果位于 `module1-basic-testing/tests/target/surefire-reports/`，包含 XML 和文本报告。`target/` 不纳入 Git；`clean` 会清除上次构建结果，需要保留的证据应另存。已留档的常规执行日志为 [baseline-32.log](module1-basic-testing/evidence/baseline-32.log)。

## 复现已知缺陷（可选）

3 项缺陷均已确认，尚未修复和关闭。它们由独立的 `defect-probes` Maven profile 执行，不计入上述 32 条常规测试的通过率。

在 `module1-basic-testing/tests` 的 Bash 终端内，按需运行：
...
- [测试用例清单](module1-basic-testing/docs/test-cases.xlsx)
- [缺陷报告](module1-basic-testing/docs/defect-report.doc)
- [模块一测试报告](module1-basic-testing/docs/test-report.docx)
- [自动化测试工程说明](module1-basic-testing/tests/README.md)
